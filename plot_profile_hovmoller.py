...
if __name__ == '__main__':
    ...
    media_interanual = df[['spd', 'dir']].groupby(
        pd.Grouper(level='depth')).mean()

    media_interanual['dir'] *= np.nan

    media_interanual['dir'] = df['dir'].groupby(
    ...
    ax.plot(media_interanual['spd'], -(media_interanual.index.values))
    ...
    ax.set_xlim(0, media_interanual['spd'].max())
    ax.set_ylim(-media_interanual.index.values.max(), 100)
